...
class MainAlgo:
    def __init__(self):
        ...
        while ag_sucses == 'true':
            resetN(self.pokemon_is_alocated, self._edge_for_pokemon)
    ...
    for agent_id, mission_list in agents_mission.items():
        if agent_id in agent_ids and len(mission_list) < 2:
            ...
            for p_id, p_e in edge_for_pokemon.items():
                ...
                if not main_algo.pokemon_is_alocated[(src_id, '-', dest_id)]:
                    ...
                    if  not_allocate and src_id in mission_list and dest_id in mission_list:
                        main_algo.pokemon_is_alocated[(src_id, '-', dest_id)] = True
                        break # the pokemon on list of agent   #Todo: check that only internal loop is break
            ...
            if at_end:
                ...
                main_algo.pokemon_is_alocated[(min_src_id, '-', min_dest_id)] = True
...
def resetN(pokemon_is_alocated, edge_for_pokemon):
        d = []
        for p_id, p_e in edge_for_pokemon.items():
            src_id = p_e.get_src()
            dest_id = p_e.get_dest()
            i = src_id, '-', dest_id
            d.insert(0, i)
        for id in pokemon_is_alocated:
            if d.__contains__(id):
                continue
            else:
                pokemon_is_alocated[id] = False
    # for p_id, p_e in edge_for_pokemon.items():
    #     src_id = p_e.get_src()
    #     dest_id = p_e.get_dest()
